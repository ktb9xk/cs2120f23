...
-- Here:
def prod_com_reverse {αbeta : Type} : β × α → α × β
| (b, a) => (a, b)
/-! 
...
-- Here
def its_wet {rain sprinkler wet : Type} (rs : rain ⊕ sprinkler) (rain_wet : rain → wet) (sprinkler_wet : sprinkler → wet) : wet :=
match rs with
| Sum.inl r => rain_wet r
| Sum.inr s => sprinkler_wet s
...
-- Here:
def sum_elim {α β γ : Type} (ab : α ⊕ β) (a_2_c : α → γ) (b_2_c : β → γ) : γ :=
match ab with
| Sum.inl a => a_2_c a
| Sum.inr b => b_2_c b
